    ...
    if not request.user.is_authenticated():
        ...
        if form.is_valid():
            ...
            return render(request, 'index/detail.html',
                          {'student': student, 'course': course, 'topics': topics, 'all_assignments': all_assignments,
...
def add_course_material(request,topic_id):
    if not request.user.is_authenticated():
        ...
        return render(request, 'index/update_course.html',{'student': student,'form':form,'topic': topic,'x':topic})

def take_quiz(request,quiz_id):
    return render(request,'index:take_quiz.html')
...
def add_resourse(request,topic_id):
    if not request.user.is_authenticated():
        ...
        if form.is_valid():
            ...
            topics = Topic.objects.filter(course=course)
            return render(request, 'index/detail.html',
                          {'student': student, 'course': course,'topics':topics, 'form': form, 'x': topic,'action_view': 'index:add_resource'})
        return render(request, 'index/update_course.html',
                          {'student': student, 'form': form, 'topic': topic, 'x': topic,
                           'block_title': topic.topic_name + ': Add Quiz', 'action_view': 'index:add_resourse',})
...
def add_quiz(request,topic_id):
    if not request.user.is_authenticated():
        ...
        if form.is_valid():
            ...
            topics = Topic.objects.filter(course=course)
            return render(request, 'index/detail.html',
                          {'student': student, 'course': course, 'topics': topics, 'form':form,'x':topic})
        return render(request, 'index/update_course.html',{'student': student,'form':form,'topic': topic,'x':topic,
                                                           'block_title': topic.topic_name + ': Add Quiz','action_view':'index:add_quiz',})
...
def add_question(request,quiz_id):
    if not request.user.is_authenticated():
        ...
        if form.is_valid():
            ...
            #a.is_subjective = request.POST['is_subjective']
            a.option1 = request.POST['option1']
            a.option2 = request.POST['option2']
            a.option3 = request.POST['option3']
            a.option4 = request.POST['option4']
            a.save()
            return render(request, 'index/quiz_page.html',
                          {'student': student, 'quiz': quiz,'form':form,'x':quiz,
                           'block_title': quiz.quiz_name + ': Add Question','action_view':'index:add_question'})
        return render(request, 'index/update_course.html',{'student': student,'form':form,
                                                           'block_title': quiz.quiz_name + ': Add Question','quiz': quiz,'x':quiz,'action_view':'index:add_question'})
...
def quiz_page(request, quiz_id):
    if not request.user.is_authenticated():
        ...
        student = get_object_or_404(Student, login_id=request.user.username)
        quiz = get_object_or_404(Quiz, pk=quiz_id)
        questions = Question.objects.filter(parent_quiz=quiz)
        context = {'questions': questions, 'quiz': quiz, 'student':student}
        return render(request,'index/quiz_page.html', context)
...
def quiz_report(request,quiz_id):
    if not request.user.is_authenticated():
        ...
        questions = Question.objects.filter(parent_quiz=quiz)
        context = {'questions': questions, 'quiz': quiz, 'student': student}
        context.update({'answers':request.POST})
        return render(request, 'index/quiz_report.html', context)
